...
if __name__=='__main__':
    ...
    p.add_argument('--precision',choices=['fp32','bf16'],default='bf16');p.add_argument('--device',default='auto',help='auto (GPU if available, else CPU), cpu or cuda[:N]')
    p.add_argument('--disable-native-triton',action='store_true')
    a=p.parse_args()
    from predict_toy_decisions import DecisionPredictor
    engine=DecisionPredictor(a.checkpoint_dir,device_name=a.device,precision=a.precision,disable_native_triton=a.disable_native_triton)
    ...
    print(json.dumps({'url':f'http://{a.host}:{a.port}','ready':True,'device':str(engine.device),'precision':engine.precision,'provider_calls':0}),flush=True)
